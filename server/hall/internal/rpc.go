...
func RpcHandler(rc *rpc.RpcContext) {
	...
	fn := func() {
		...
		if rc.ReplyErr == nil {
			mlog.Infof("handler msg succeed, method:%s, req_data:%v, rsp_data:%v", rc.Req.MethodName, argMsg, rc.Reply)
		} else {
			mlog.Errorf("handler msg failed, method:%s, req_data:%v, err:%v", rc.Req.MethodName, argMsg, rc.ReplyErr)
		}
	}
